...
def maxwell(E):
    return np.sqrt(E-B) * np.exp(-(E-B)/T)
...
bins = np.linspace(int(min(sp)), int(max(sp)), 76)
ax4.hist(sp, edgecolor='k', density=True, bins=bins)
fig4.savefig(f'{plots}/stoppingpower.png')


fig5, ax5 = plt.subplots(figsize=(19.20, 10.80))
ax5.plot(gg, sp)
fig5.savefig(f'{plots}/stoppingvenergy.png')
